...
if [[ -z $INFRA_EXISTS ]]; then
    ...
    $KUBECTL rollout restart -n "${CAPV_NAMESPACE}" deployment "${CAPI_DEPLOYMENT}"
    $KUBECTL rollout restart -n "${CAPV_NAMESPACE}" deployment "${CABPK_DEPLOYMENT}"
    $KUBECTL rollout restart -n "${CAPV_NAMESPACE}" deployment "${KCP_DEPLOYMENT}"

    $KUBECTL rollout status -n "${CAPV_NAMESPACE}" deployment "${KCP_DEPLOYMENT}"
    $KUBECTL rollout status -n "${CAPV_NAMESPACE}" deployment "${CABPK_DEPLOYMENT}"
    $KUBECTL rollout status -n "${CAPV_NAMESPACE}" deployment "${CAPI_DEPLOYMENT}"
