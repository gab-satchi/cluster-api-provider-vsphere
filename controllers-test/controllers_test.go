...
// Creates and deploys a Cluster and WCPCluster in order. Function does not
// block on WCPCluster creation.
...
// Creates and deploys a WCPMachine. Function does not block on Machine
// creation
...
var _ = Describe("Reconciler tests", func() {
	...
	assertEventuallyVMStatus := func(key client.ObjectKey, obj client.Object, expectedState infrav1.VirtualMachineState) {
		EventuallyWithOffset(1, func() (infrav1.VirtualMachineState, error) {
			if err := k8sClient.Get(ctx, key, obj); err != nil {
				return "", err
			}
			vSphereMachine := obj.(*infrav1.VSphereMachine)
	...
	assertEventuallyControlPlaneEndpoint := func(key client.ObjectKey, obj client.Object, expectedIP string) {
		EventuallyWithOffset(1, func() (string, error) {
			...
			vsphereCluster := obj.(*infrav1.VSphereCluster)
	...
	Specify("WCPClusters can be deleted without a corresponding Cluster", func() {
		...
		infraCluster.SetFinalizers([]string{infrav1.ClusterFinalizer})
	...
	Specify("WCPMachines can be deleted without a corresponding Machine or Cluster", func() {
		By("Creating an infrastructure machine with no owner or cluster reference")
		infraMachine := newAnonInfraMachine()
		Expect(k8sClient.Create(ctx, infraMachine)).To(Succeed())
		infraMachineKey := client.ObjectKey{Namespace: infraMachine.GetNamespace(), Name: infraMachine.GetName()}
		assertEventuallyFinalizers(infraMachineKey, infraMachine)

		By("Deleting the infrastructure machine and waiting for it to be removed")
		deleteAndWait(infraMachineKey, infraMachine, false)
	})

	Specify("Create and Delete a wcpMachine with a Machine but without a Cluster", func() {
		By("Create the CAPI Machine and wait for it to exist")
		// A finalizer is added to prevent it from being deleted until its
		// dependents are removed.
		machine := &clusterv1.Machine{
	...
	Specify("A VM gets properly reconciled for a Machine and reflects appropriate VM status", func() {
		...
		By("Expect the WCPMachine to have its Status.VMStatus initialized to a new VM")
		assertEventuallyVMStatus(infraMachineKey, infraMachine, infrav1.VirtualMachineStatePending)
		...
		By("Expect the WCPMachine VM status to reflect VM Created status")
		assertEventuallyVMStatus(infraMachineKey, infraMachine, infrav1.VirtualMachineStateCreated)

		By("Modifying the VM to simulate it having been powered on")
		newVM.Status.PowerState = vmoprv1.VirtualMachinePoweredOn
		Expect(k8sClient.Status().Update(ctx, newVM)).Should(Succeed())

		By("Expect the WCPMachine VM status to reflect VM PoweredOn status")
		assertEventuallyVMStatus(infraMachineKey, infraMachine, infrav1.VirtualMachineStatePoweredOn)
		...
		By("Expect the WCPMachine VM status to reflect VM Ready status")
		assertEventuallyVMStatus(infraMachineKey, infraMachine, infrav1.VirtualMachineStateReady)
