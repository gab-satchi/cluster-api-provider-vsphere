...
func assertEventuallyExists(manager *TestManager, resource schema.GroupVersionResource, name, ns string, ownerRef *metav1.OwnerReference) *unstructuredv1.Unstructured {
	var obj *unstructuredv1.Unstructured
	EventuallyWithOffset(1, func() (bool, error) {
		output, err := manager.client.Resource(resource).Namespace(ns).Get(manager.GetContext(), name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		if ownerRef != nil {
...
func assertEventuallyDoesNotExist(manager *TestManager, resource schema.GroupVersionResource, name, ns string) {
	EventuallyWithOffset(1, func() (bool, error) {
		_, err := manager.client.Resource(resource).Namespace(ns).Get(manager.GetContext(), name, metav1.GetOptions{})
		if err != nil {
