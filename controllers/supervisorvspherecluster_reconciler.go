...
type supervisorClusterReconciler struct {
...
// TODO: re-evaluate rbac needed
// +kubebuilder:rbac:groups=core,resources=secrets,verbs=get;list;watch;create;patch
// +kubebuilder:rbac:groups=core,resources=namespaces,verbs=get;list;watch
// +kubebuilder:rbac:groups=infrastructure.cluster.vmware.com,resources=vsphereclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=infrastructure.cluster.vmware.com,resources=vsphereclusters/status,verbs=get;update;patch
...
func (r supervisorClusterReconciler) Reconcile(ctx goctx.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
...
func (r *supervisorClusterReconciler) reconcileDelete(ctx *vmware.ClusterContext) reconcile.Result {
	...
	controllerutil.RemoveFinalizer(ctx.VSphereCluster, vmwarev1b1.ClusterFinalizer)

	return reconcile.Result{}
}

func (r *supervisorClusterReconciler) reconcileNormal(ctx *vmware.ClusterContext) (reconcile.Result, error) {
	ctx.Logger.Info("Reconciling vsphereCluster")

	// If the vsphereCluster doesn't have our finalizer, add it.
	controllerutil.AddFinalizer(ctx.VSphereCluster, vmwarev1b1.ClusterFinalizer)
	...
	if err != nil {
		conditions.MarkFalse(ctx.VSphereCluster, vmwarev1b1.ResourcePolicyReadyCondition, vmwarev1b1.ResourcePolicyCreationFailedReason, clusterv1.ConditionSeverityWarning, err.Error())
		return reconcile.Result{}, errors.Wrapf(err,
			"failed to configure resource policy for vsphereCluster %s/%s",
			ctx.VSphereCluster.Namespace, ctx.VSphereCluster.Name)
	}
	conditions.MarkTrue(ctx.VSphereCluster, vmwarev1b1.ResourcePolicyReadyCondition)
...
func (r *supervisorClusterReconciler) reconcileControlPlaneEndpoint(ctx *vmware.ClusterContext) (bool, error) {
	if !ctx.Cluster.Spec.ControlPlaneEndpoint.IsZero() {
		...
		if r.networkProvider.HasLoadBalancer() {
			conditions.MarkTrue(ctx.VSphereCluster, vmwarev1b1.LoadBalancerReadyCondition)
		}
...
func (r *supervisorClusterReconciler) reconcileLoadBalancedEndpoint(ctx *vmware.ClusterContext) error {
...
func (r *supervisorClusterReconciler) reconcileAPIEndpoints(ctx *vmware.ClusterContext) error {
