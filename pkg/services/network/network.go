...
const (
	...
	CAPWDefaultNetworkLabel    = "capw.vmware.com/is-default-network" //nolint:gosec
...
func (np *netopNetworkProvider) getDefaultClusterNetwork(ctx *vmware.ClusterContext) (*netopv1.Network, error) {
	...
	case 0:
		return nil, fmt.Errorf("no default CAPW Network found")
	case 1:
		return &networkList.Items[0], nil
	default:
		return nil, fmt.Errorf("more than one default CAPW Network found: %d", len(networkList.Items))
	}
}
